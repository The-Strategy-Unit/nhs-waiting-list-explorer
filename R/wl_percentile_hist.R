...
wl_percentile_hist <- function(wl_hist, percentage = 92) {
    ...
    # Calculate actual number of days in the bin
    bin_days <- as.numeric(difftime(row_p$arrival_before, row_p$arrival_since, units = "days"))
    
    # Calculate date assuming even distribution within the bin
    days_to_add <- prop_in_bin * bin_days
    date_p <- row_p$arrival_since + as.difftime(days_to_add, units = "days")
    
    # Calculate weeks as non-integer value
    weeks_p <- as.numeric(difftime(row_p$report_date, date_p, units = "weeks"))
